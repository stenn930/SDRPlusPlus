...
SDRPP_MOD_INFO{
    /* Name:            */ "fobosagilesdr_source",
    /* Description:     */ "FobosAgileSDR Source Module",
    /* Author:          */ "sten",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ -1
};
...
#define FOBOS_AGILE_LNA_GAIN_MIN  1 // from doc: "low noise amplifier 0..2" but seems to be 1..3 in reality 
#define FOBOS_AGILE_LNA_GAIN_MAX  3
#define FOBOS_AGILE_VGA_GAIN_MIN  0 // from doc: "variable gain amplifier 0..15"
#define FOBOS_AGILE_VGA_GAIN_MAX  15 

class FobosAgileSDRSourceModule : public ModuleManager::Instance {
public:
    FobosAgileSDRSourceModule(std::string name) {
        this->name = name;
        ...
        sigpath::sourceManager.registerSource("FobosAgileSDR", &handler);
    }

    ~FobosAgileSDRSourceModule() {
        // Nothing to do
    }
...
private:
    ...
    void refresh() {
        ...
        int devCount = fobos_sdr_list_devices(serials);
        if (devCount <= 0) {
            flog::error("Failed to list FobosAgile device list: {}", devCount);
            return;
        }
    ...
    void select(const std::string& serial) {
        ...
        err = fobos_sdr_get_samplerates(dev, srList, &srCount);
        if (err) {
        ...
        // Define clock sources -- not using due to a known bug in Fobos Agile
        // clockSources.clear();
        ...
        if (config.conf["devices"][selectedSerial].contains("lnaGain")) {
            lnaGain = std::clamp<int>(config.conf["devices"][selectedSerial]["lnaGain"], FOBOS_AGILE_LNA_GAIN_MIN, FOBOS_AGILE_LNA_GAIN_MAX);
        }
        if (config.conf["devices"][selectedSerial].contains("vgaGain")) {
            vgaGain = std::clamp<int>(config.conf["devices"][selectedSerial]["vgaGain"], FOBOS_AGILE_VGA_GAIN_MIN, FOBOS_AGILE_VGA_GAIN_MAX);
        }
        config.release();
    ...
    static void menuSelected(void* ctx) {
        FobosAgileSDRSourceModule* _this = (FobosAgileSDRSourceModule*)ctx;
        core::setInputSampleRate(_this->sampleRate);
        flog::info("FobosAgileSDRSourceModule '{0}': Menu Select!", _this->name);
    }

    static void menuDeselected(void* ctx) {
        FobosAgileSDRSourceModule* _this = (FobosAgileSDRSourceModule*)ctx;
        flog::info("FobosAgileSDRSourceModule '{0}': Menu Deselect!", _this->name);
    }
    ...
    static void start(void* ctx) {
        ...
        // Open the device
        int err = fobos_sdr_open(&_this->openDev, _this->selectedDevId);
        if (err) {
            flog::error("Failed to open device: {}", err);
            return;
        }
        ...
        double actualSr = (_this->sampleRate <= 50e6) ? _this->sampleRate : 50e6;
        fobos_sdr_set_samplerate(_this->openDev, actualSr);
        fobos_sdr_set_frequency(_this->openDev, _this->freq);
        fobos_sdr_set_direct_sampling(_this->openDev, _this->port != PORT_RF);
        ...
        // Start worker
        _this->run = true;
        _this->workerThread = std::thread(&FobosAgileSDRSourceModule::worker, _this);
        
        _this->running = true;
        flog::info("FobosAgileSDRSourceModule '{0}': Start!", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        flog::info("FobosAgileSDRSourceModule '{0}': Stop!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        _this->freq = freq;
        flog::info("FobosAgileSDRSourceModule '{0}': Tune: {1}!", _this->name, freq);
    }
    ...
    static void menuHandler(void* ctx) {
        ...
        if (_this->port == PORT_RF) {
            ...
            SmGui::FillWidth();
            if (SmGui::SliderInt(CONCAT("##_fobossdr_lna_gain_", _this->name), &_this->lnaGain, FOBOS_AGILE_LNA_GAIN_MIN, FOBOS_AGILE_LNA_GAIN_MAX)) {
                if (_this->running) {
                    fobos_sdr_set_lna_gain(_this->openDev, _this->lnaGain);
                }
                if (!_this->selectedSerial.empty()) {
            ...
            SmGui::FillWidth();
            if (SmGui::SliderInt(CONCAT("##_fobossdr_vga_gain_", _this->name), &_this->vgaGain, FOBOS_AGILE_VGA_GAIN_MIN, FOBOS_AGILE_VGA_GAIN_MAX)) {
                if (_this->running) {
                    fobos_sdr_set_vga_gain(_this->openDev, _this->vgaGain);
                }
                if (!_this->selectedSerial.empty()) {
...
MOD_EXPORT void _INIT_() {
    ...
    def["devices"] = json({});
    def["device"] = "";
    config.setPath(core::args["root"].s() + "/fobosagilesdr_config.json");
    config.load(def);
    config.enableAutoSave();
}

MOD_EXPORT ModuleManager::Instance* _CREATE_INSTANCE_(std::string name) {
    return new FobosAgileSDRSourceModule(name);
}

MOD_EXPORT void _DELETE_INSTANCE_(void* instance) {
    delete (FobosAgileSDRSourceModule*)instance;
}
